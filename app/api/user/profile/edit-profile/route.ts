...
export async function POST(request : Request) {
    ...
    const gender : any = userProfile?.gender ?? "";
    const cartUid : any = userProfile?.cartId ?? "";
    const email : any = userProfile?.email ?? "";
    ...
    const newProfile : Profile = {
        ...
        email: email,
        cartId: cartUid,
        profileImg : requestBody?.profileImg ?? "",
        phoneNum : requestBody?.phoneNum ?? "",
        address : requestBody?.address ?? "",
        gender : gender
    }
    
    await profileController.editProfile(newProfile);
